...
function initProposalState(channelId) {
  ...
  proposalState.items = {};
  proposalState.termsPresetId = null;
...
function getActiveTermsPreset() {
  const presets = getChannelTermPresets(appData, proposalState.channelId);
  if (presets.length === 0) return null;
  return presets.find((p) => p.id === proposalState.termsPresetId) || presets[0];
}

function getResolvedProposalTerms() {
  return getActiveTermsPreset()?.lines ?? [];
}
...
function renderProposal() {
  ...
  const termPresets = getChannelTermPresets(appData, proposalState.channelId);
  const activePreset = getActiveTermsPreset();
    ...
    <div class="terms-box">
      ...
      ${
        termPresets.length > 0
          ? `
        <div class="terms-preset-tabs no-print">
          ${termPresets
            .map(
              (p) =>
                `<button type="button" class="terms-preset-tab ${p.id === activePreset?.id ? "active" : ""}" data-preset-id="${p.id}">${escapeAttr(p.name)}</button>`
            )
            .join("")}
        </div>`
          : ""
      ...
      <p class="no-print" style="margin-top:12px;font-size:13px;color:var(--text-muted)">
        ...
        조건 세트가 여러 개면 위 버튼에서 이 단가표에 적용할 조건을 선택할 수 있습니다.
      </p>
    </div>
  `;
}
...
function bindProposalEvents() {
  ...
  document.querySelectorAll(".terms-preset-tab").forEach((tab) => {
    tab.addEventListener("click", (e) => {
      proposalState.termsPresetId = e.target.dataset.presetId;
      render();
    });
  });
...
async function savePoUpload() {
  ...
  const terms = getChannelTermPresets(appData, form.channelId)[0]?.lines || [];
  const products = getProducts(appData);
  let items = [];
...
function renderMasterChannelDetail(channelId) {
  ...
  const clients = getClients(appData, channelId);
  const terms = getChannelTermPresets(appData, channelId);
  ...
  return `
    ...
    <div class="card">
      ...
      <p class="card-desc">
        조건 세트를 여러 개 만들어두면 단가표 작성 시 상황에 맞는 세트를 골라 쓸 수 있습니다. (예: 해외 수출용, 국내용)
        각 세트는 한 줄에 한 항목씩 입력하세요.
      </p>
      <form id="master-terms-form" class="term-presets-form">
        ${
          terms.length === 0
        ...
        ${terms
          .map(
            (preset, idx) => `
          <div class="term-preset">
            <div class="term-preset-header">
              <input type="text" class="term-preset-name" data-preset-index="${idx}" value="${escapeAttr(preset.name)}" placeholder="조건 세트 이름 (예: 해외 수출)">
              <button type="button" class="btn btn-secondary btn-sm" data-preset-remove="${idx}">삭제</button>
            </div>
            <textarea class="terms-editor term-preset-editor" data-preset-index="${idx}" rows="${Math.max(
              4,
              preset.lines.length
            )}" placeholder="1. 납품 조건 : ...&#10;2. 결제 조건 : ...">${preset.lines.join("\n")}</textarea>
          </div>`
          )
          .join("")}
        <button type="button" class="btn btn-secondary" id="btn-master-add-term">+ 조건 세트 추가</button>
        <div class="master-form-actions">
        ...
        ${
          terms.length
            ? terms
                .map(
                  (preset) => `
              <div class="term-preset-preview">
                <p class="term-preset-preview-name">${escapeAttr(preset.name)}</p>
                ${preset.lines.map((l) => `<p>${l}</p>`).join("")}
              </div>`
                )
                .join("")
...
function bindMasterEvents() {
  ...
  function readTermPresetsFromMasterForm() {
    return Array.from(document.querySelectorAll(".term-preset-editor")).map((el, i) => {
      const idx = el.dataset.presetIndex;
      const nameInput = document.querySelector(`.term-preset-name[data-preset-index="${idx}"]`);
      return {
        name: nameInput?.value.trim() || `조건 ${i + 1}`,
        lines: el.value
          .split("\n")
          .map((line) => line.trim())
  ...
  document.getElementById("master-terms-form")?.addEventListener("submit", (e) => {
    e.preventDefault();
    const terms = readTermPresetsFromMasterForm().filter((p) => p.lines.length > 0);
  ...
  document.getElementById("btn-master-add-term")?.addEventListener("click", () => {
    const terms = readTermPresetsFromMasterForm();
    terms.push({ name: `조건 ${terms.length + 1}`, lines: [""] });
    setChannelTerms(appData, masterChannelId, terms);
    render();
  });

  document.querySelectorAll("[data-preset-remove]").forEach((btn) => {
    btn.addEventListener("click", () => {
      const idx = parseInt(btn.dataset.presetRemove, 10);
      const terms = readTermPresetsFromMasterForm();
      terms.splice(idx, 1);
      setChannelTerms(appData, masterChannelId, terms);
      render();
  ...
  document.getElementById("btn-master-reset-terms")?.addEventListener("click", async () => {
    ...
    setChannelTerms(appData, masterChannelId, getDefaultChannelTermPresets(appData, masterChannelId));
